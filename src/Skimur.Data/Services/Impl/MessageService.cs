...
namespace Skimur.Data.Services.Impl
{
    public class MessageService : IMessageService
    {
        ...
        public void InsertMention(Guid userId, Guid authorId, Guid? postId, Guid? commentId)
        {
            if (postId.HasValue && commentId.HasValue)
            {
                throw new Exception("You must pass either post, or comment, but not both.");
            }

            if (!postId.HasValue && !commentId.HasValue)
            {
                throw new Exception("You must provide a post or comment id.");
            }
