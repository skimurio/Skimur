...
namespace Skimur.Web.Infrastructure.Identity
{
        ...
        IUserTwoFactorStore<User>
    {
        ...
        public ApplicationUserStore(IMembershipService membershipService, IPasswordManager passwordManager)
        {
        ...
        Task<IdentityResult> IUserStore<User>.CreateAsync(User user, CancellationToken cancellationToken)
        {
            return Task.Factory.StartNew(() =>
            {
                ...
                return IdentityResult.Failed(new IdentityError { Description = "Could Not Create User" });
            });
        }
        ...
        public Task<IdentityResult> CreateAsync(Role role, CancellationToken cancellationToken)
        {
            return Task.Factory.StartNew(() =>
            {
                if (_membershipService.InsertRole(role))
                {
                    return IdentityResult.Success;
                }

                return IdentityResult.Failed(new IdentityError { Description = "Failed to add role." });
            });
        }
