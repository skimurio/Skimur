...
namespace Skimur.Web.Utils
{
    public static class AvatarBuilder
    {
        public static void Build(Stream source, string outFile)
        {
            using (var img = Image.Load(source))
            {
                // as generate returns a new IImage make sure we dispose of it
                using (Image destAvatar = img.Clone(x => x.ConvertToAvatar(new Size(150, 150), 20)))
                {
                    destAvatar.Save(outFile);
                }
            }
        }
